...
Each type of [`Result`](xref:rachis-glossary-target#term-result) (i.e., [Artifacts and Visualizations](#getting-started:artifacts-and-visualizations)) and [`Action`](xref:rachis-glossary-target#term-action) (i.e., [methods](xref:rachis-glossary-target#term-method), [visualizers](xref:rachis-glossary-target#term-visualizer), and [pipelines](xref:rachis-glossary-target#term-pipeline)) is represented by a different color-coded node.
...
1. Metadata is a central concept in QIIME 2.
   ...
   Instead, find discussion of metadata in [our documentation of the metadata file format](xref:using-rachis-target#metadata-formatting-reference).
...
Include this barcode information in your [sample metadata](xref:using-rachis-target#metadata-formatting-reference) file.
...
Hint: read about [using Artifacts as metadata](xref:using-rachis-target#view-artifacts-as-metadata) - this is a very powerful concept in QIIME 2, and opens up many diverse analysis possibilities.
:::
...
A [stand-alone plugin](xref:rachis-glossary-target#term-stand-alone-plugin), [`q2-boots`](https://library.qiime2.org/plugin/caporaso-lab/q2-boots) has recently (as of March 2025) been released that facilitates bootstrapped and rarefaction-based diversity analysis.
...
QIIME 2 continues to grow, so stay tuned for more plugins in future releases 📻, and keep your [eyes peeled](https://library.qiime2.org) for [stand-alone plugins](xref:rachis-glossary-target#term-stand-alone-plugin) that will continue to expand the functionality availability in QIIME 2.
