...
If you need to process your data more interactively, you might want to use the Python 3 API, and/or an [Artifact Cache](xref:using-rachis-target#artifact-cache-tutorial).
...
The metadata file format is defined in *Using QIIME 2* [here](xref:using-rachis-target#metadata-formatting-reference).
You can find an example in the *Gut-to-Soil Tutorial* [here](xref:gut-to-soil-target#gut-to-soil-tutorial:sample-metadata).
...
That said, the deblur developers recommend doing an initial quality screen - this is illustrated in the *Moving Pictures tutorial's* [Deblur section](xref:moving-pictures-target#deblur).
