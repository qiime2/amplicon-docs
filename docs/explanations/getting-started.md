...
- `.qza` files store QIIME 2 {term}`Artifacts <artifact>` on disk.
...
- `.qzv` files store QIIME 2 {term}`Visualizations <visualization>` on disk.
...
- `.qza` and `.qzv` files can be loaded with [QIIME 2 View](https://view.qiime2.org). Other options for viewing `.qza` and `.qzv` files are discussed [here](https://use.qiime2.org/en/2026.1/how-to-guides/view-visualizations.html).
...
Methods are usually thought of intermediary analysis steps: they take {term}`artifacts <artifact>` and/or metadata as input, and they generate one or more artifacts as output.
Visualizers represent terminal steps in an analysis: they take {term}`artifacts <artifact>` and/or metadata as input, and they generate one or more {term}`visualizations <visualization>` as output.
...
If you'd like to learn more about Artifact Classes, see [*Semantic types, data types, file formats, and artifact classes*](https://develop.qiime2.org/en/2026.1/plugins/explanations/types-of-types.html) in *Developing with QIIME 2*.
