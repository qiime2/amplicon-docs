...
- `.qza` files store QIIME 2 [Artifacts](xref:rachis-glossary-target#term-artifact) on disk.
...
- `.qzv` files store QIIME 2 [Visualizations](xref:rachis-glossary-target#term-visualization) on disk.
...
- `.qza` and `.qzv` files can be loaded with [QIIME 2 View](https://view.qiime2.org). Other options for viewing `.qza` and `.qzv` files are discussed [here](xref:using-rachis-target#view-visualizations).
...
All of the analysis functionality in QIIME 2 comes in the form of [plugins](xref:rachis-glossary-target#term-plugin).

Plugins define [actions](xref:rachis-glossary-target#term-action), which are the individual commands that you'll run in an analysis workflow.
...
Three types of actions can be defined by plugins: [methods](xref:rachis-glossary-target#term-method), [visualizers](xref:rachis-glossary-target#term-visualizer), and [pipelines](xref:rachis-glossary-target#term-pipeline).
Methods are usually thought of intermediary analysis steps: they take [artifacts](xref:rachis-glossary-target#term-artifact) and/or metadata as input, and they generate one or more artifacts as output.
Visualizers represent terminal steps in an analysis: they take [artifacts](xref:rachis-glossary-target#term-artifact) and/or metadata as input, and they generate one or more [visualizations](xref:rachis-glossary-target#term-visualization) as output.
Pipelines combine calls to other actions in a single action, and are often used to make repetitive sub-workflows easier to run.
They take artifacts and/or metadata as input, and they generate one or more artifacts and/or visualizations as output.
