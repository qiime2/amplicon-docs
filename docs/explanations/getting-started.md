...
`.qza` files are also referred to as QIIME 2 Artifacts, and these are data files that are generated by QIIME 2 and intended to be used by QIIME 2.
`.qzv` files are also referred to as QIIME 2 Visualizations.
These are data files that are generated by QIIME 2 and intended to be viewed by humans, for example using [QIIME 2 View](https://view.qiime2.org).
