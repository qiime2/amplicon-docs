...
This is of grave concern particularly in low-biomass environments where the impact of these contaminants is heightened due to the low amount of starting biological material from the target community.
Decontam seeks to remedy this by removing contaminants at the feature level before relative abundance calculation.
This tool was developed and maintained by [Benjamin Callahan](https://github.com/benjjneb/decontam) and has been shown to be effective in identifying contaminants in datasets of various structures.
Here we present Decontam functionality within the `q2-quality-control` plugin of the QIIME 2 platform, which for the rest of this document will be referred to as `q2-Decontam`.
...
If you're completely new to QIIME 2, we recommend reading [Getting Started with QIIME 2](https://amplicon-docs.qiime2.org/en/latest/explanations/getting-started.html) to familiarize yourself with concepts that may be helpful. To install a local version of QIIME 2 follow the instructions [here](https://library.qiime2.org/quickstart).

## Installation and base actions

The `q2-Decontam` functionality is included by default in the Amplicon and MOSHPIT distributions under the `q2-quality-control` plugin.
`q2-Decontam` consists of two main actions:
...
**Note**: The frequency method is the only decontamination method that can be used when there are no control samples.
...
Note that only one of these methods are alternative options and only one is needed to identify contaminants in your samples.

### Decontam-Score-Viz

The second step in the q2-Decontam workflow is `decontam-score-viz`.
...
This guide was originally written by Jorden Rabasco, who can be reached at [jrabasc\@ncsu.edu](mailto:jrabasc@ncsu.edu), and whose GitHub page can be found [here](https://github.com/jordenrabasco).
